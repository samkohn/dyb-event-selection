...
def chi_square(constants, fit_params, return_array=False, debug=False):
    ...
    if debug:
        pprint(observed)
        pprint(predicted)
    return_array_values = np.zeros((16,))
    term_index = 0
...
def residual_fn(x, constants):
    ...
    return np.hstack((residuals[:4], residuals[-4:]))
...
def fit_least_squares(starting_params, constants):
    """Perform the fit using least squares."""

    x0 = [starting_params.theta13] + starting_params.to_list()[-4:]
    result = least_squares(residual_fn, np.array(x0),
            args=(constants,), method='trf')
    return result
...
def chi_square_grid(starting_params, constants, theta13_values):
    ...
    for i, theta13 in enumerate(theta13_values):
        ...
        fit_result = fit_lsq_frozen(starting_params, constants,
                [0] + list(range(1, 9)))
